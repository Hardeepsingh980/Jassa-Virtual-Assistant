...
class Widget():
    ...
    #------------------------------------------------------KEY WORD LISTS---------------------------------------------------------------------------
    #                THESE ARE ALL THE LIST WHICH CONTAINS IMPORTANT KEYWORDS WHICH THE Jassa CAN USE.
    #--------------------------------------------------------------------------------------------------------------------------------------------------------
    greet = ['Hey Whats up! How are you doing', 'Hello Hardeep','Hi ! I am your Assistant','Hello, How can I help You']
    how = ['I am Fine Sir, What about You','I am fine as always', 'what you think','just fine!!']
    name = ['You Can call me Jassaa','My Name Is Jassaa','you named me Jassaa','You Should Know this, My name Is Jassaa']
    creator = ['You made Me','I was Made by Hardeep Singh','One And only, Hardeep Singh','Best in the World, Hardeep Singh']
    ...
    me = ['You Told Me Your Name, Hardeep Singh','I Think That\'s Hardeep Singh','Your Name That I Know Is Hardeep Singh']
    ...
    #---------------------------PRINTING FUNC---------------------------------------------------
    #       THIS FUCTION PRINTS THE Jassa'S RESPONCE TO SCREEN.
    #---------------------------------------------------------------------------------------------------
    ...
    #-----------------SPEAK FUNCTION-------------------------------
    #    THIS FUNCTION SPEAKS THE Jassa'S RESPONCE
    #------------------------------------------------------------------------
    def speak(self,s):
        self.engine.say(s)
    ...
    #-----------------------SEND FUNC-----------------------------------------------------------------
    #        THIS FUCTION IS BEING USED TO SEND USER_INPUT TO Jassa.
    #--------------------------------------------------------------------------------------------------------
    def send_func(self):
    ...
    #-------------------------------------------------------------------------------------------------------
        ...
        #------------------------------------CONDITIONS------------------------------------------------------------------------------------------
        # THESE ARE CONDITIONS WHICH THE Jassa CHECKS. HE RESPOND TO THE MOST RELEVENT CONDITION.
        #-------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def process_user_input(self, user_input):
        ...
        #-----------------GURPARTAP CONDITION-------------------------------------------------------------------
        elif 'gurpartap'  in user_input:
            out = 'Gurpartap Singh Is A Good Friend Of Hardeep Singh. He is well Known For HIs Gaups'                                                                                               #|   
            self.printing_func(out)                                                                                    #|
            self.speak(out)
        #-----------------------------------------------------------------------------------------------------------------------
        ...
        elif user_input in ['amazing', 'great','nice']:
            out = 'Hmmmm'                                                                                              #|   
            self.printing_func(out)                                                                                    #|
            self.speak(out)
       ...
       #--------------------------THANKS CONDITION-------------------------------------------------------   
        ...
        #-----------------------------GOOD MORNING CONDITION--------------------------------------------------------------------------------
            ...
            out = f'Good Morning Hardeep, The Current Time is {time},  And {outa},    Have A Good Day Sir.'
            self.printing_func(out)
            self.speak(out)
        #--------------------------------------------------------------------------------------------------------------------------------------------------------            
    ...
    def threaded_send_func(self):
        _thread.start_new_thread(self.send_func, ())

    def threaded_listen_func(self):
        _thread.start_new_thread(self.listen_from_mic, ())
    ...
    def listen_from_mic(self):
        try:
            with self.mic as source:
                self.printing_func('LISTENING.....')
                # self.r.adjust_for_ambient_noise(source)
                audio = self.r.record(source=source, duration=3)

            self.printing_func('PROCESSING....')

            self.search_var.set('')
            self.text_box1.delete(1.0,END)

            user_input = self.r.recognize_google(audio).lower()
            self.text_box1.insert(INSERT,user_input)
            self.process_user_input(user_input)
        except:
            user_input = ''
    ...
    def __init__(self):
        ...
        Label(self.win, text='Jassa' , font=('arial black',20),fg='white',bg='orange').place(x=260,y=50)
        ...
        send = Button(self.win, text='Send', font=('arial black',10),bg='blue',fg='white',bd=5,width=5,command=self.threaded_send_func).place(x=270,y=250)
        send = Button(self.win, text='Mic', font=('arial black',10),bg='blue',fg='white',bd=5,width=3,command=self.threaded_listen_func).place(x=330,y=250)

        def enter(*args):
            self.threaded_send_func()
